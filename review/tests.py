...
class TestViews(TestCase):
    ...
    def test_review_update(self):
        ...
        #check if the review for the current user was updated with the correct values 
        response = self.client.get('/reviews/')
        reviews = response.context['reviews']
        self.assertEqual(reviews.count(), 1)
        self.assertTrue(reviews[0].author == self.user)
        ...
        self.assertTrue((reviews[0].date_updated_on).strftime("%Y-%m-%d %H:%M:%S")  == now.strftime("%Y-%m-%d %H:%M:%S") )
