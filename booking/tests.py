...
class TestViews(TestCase):
    ...
    def test_created_booking_is_rendered_in_profile(self):
        ...
        self.assertTrue(bookings[0].date == datetime.strptime(
            "2022-08-26", "%Y-%m-%d").date())
        self.assertTrue(bookings[0].start_time == datetime.strptime(
            "16:00:00", "%H:%M:%S").time())
        self.assertTrue(bookings[0].end_time == datetime.strptime(
            "19:00:00", "%H:%M:%S").time())
    ...
    def test_profile_booking_delete(self):
        ...
        self.client.post('/bookings/profile/booking/' +
                         str(bookings[0].pk) + '/remove/')
    ...
    def test_admin_manage_bookings_filter(self):
        ...
        # check if the booking lists contains only the bookings filtered after
        # the date submited in form
        response = self.client.get('/bookings/managebookings/', form_data)
        bookings = response.context['booking_list']
        self.assertEqual(bookings.count(), 1)
        self.assertTrue(bookings[0].date == datetime.strptime(
            "2022-08-10", "%Y-%m-%d").date())

        # add a date value that doesn't exist in the booking database for the
        # form used to filter bookings
        form_data = {
            "date": datetime.strptime("2022-05-01", "%Y-%m-%d").date()
        }

        # check if the booking lists is empty because there can't be found
        # a booking for the provided date
        response = self.client.get('/bookings/managebookings/', form_data)
        bookings = response.context['booking_list']
        self.assertEqual(bookings.count(), 0)
    ...
    def test_admin_manage_bookings_delete(self):
        ...
        self.client.post('/bookings/managebookings/booking/' +
                         str(bookings[0].pk) + '/remove/')
